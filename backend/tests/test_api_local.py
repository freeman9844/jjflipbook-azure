...
@patch("routers.flipbooks.get_container")
def test_list_flipbooks_returns_signed_responsive_covers(mock_get_container, _sign_url):
    container = MagicMock()
    container.query_items.return_value = [
        {
            ...
            "uuid_key": "book-1",
            "title": "Book",
            "cover_urls": ["cover-384.webp", "cover-640.webp"],
    ...
    assert response.status_code == 200
    assert response.json()[0]["cover_urls"] == [
        "cover-384.webp?signed",
        "cover-640.webp?signed",
    ]
    assert response.json()[0]["image_urls"] == ["page-1.webp?signed"]
    query = container.query_items.call_args.kwargs["query"]
    assert "c.cover_urls" in query
...
@patch("routers.flipbooks.sign_url", side_effect=lambda value: f"{value}?signed")
@patch("routers.flipbooks.get_container")
def test_list_flipbooks_keeps_first_page_fallback_for_existing_books(
    mock_get_container,
    _sign_url,
):
    container = MagicMock()
    container.query_items.return_value = [
        {
            "id": "legacy-book",
            "uuid_key": "legacy-book",
            "title": "Legacy",
            "image_urls": ["page-1.webp"],
        }
    ]
    mock_get_container.return_value = container
...
def test_get_flipbook_keeps_complete_page_manifest(mock_get_container, _sign_url):
    ...
    assert response.json()["cover_urls"] == [
        "cover-384.webp?signed",
        "cover-640.webp?signed",
    ]
